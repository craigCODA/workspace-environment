...
On first launch, Coda speaks the welcome while synchronized captions appear at the bottom, then asks what to call you and confirms what it heard before saving. Later launches say `Welcome back, <name>.` After that, ambient speech is ignored until `Hey Coda` is detected. The **Talk** control can start the same conversation without a wake phrase, which is more reliable in a noisy room. The **Chat** panel mirrors recognized speech and Coda's spoken responses and can also send typed instructions through the same agent and approval path. Microphone, captions, transcript display, activity, proactive alerts, and navigation policy remain directly controllable in the workspace.

Speech output prefers the newer Windows OneCore media voice catalog and automatically chooses a compatible Natural or HD voice when Windows exposes one to applications. It falls back to legacy Windows speech only if OneCore playback is unavailable. Low-confidence wake and dictation fragments are discarded locally; no raw microphone audio is persisted or sent to Codex.
